...
The canonical inventory of macros and classes is `../components.yaml`.
...
**Retired — not implemented, do not use:** `@chapter-opener` (the chapter-opener composite is markup-driven now — see `05-page-templates.md`) and `@class-entry` / `@end-class-entry` (retired 2026-05-24; its CSS was parked in `css/deprecated.css`). Neither marker is recognised by the plugin, so both pass through as literal text.